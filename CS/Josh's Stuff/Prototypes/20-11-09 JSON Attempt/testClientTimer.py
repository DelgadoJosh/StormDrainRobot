...
def retrieveData(data):
  ...
  # Retrieve all the data based on the message size
...
while True:
  try:
    ...
    # dataTime = data[msg_size:]
    # # print(frame_data)


    # while len(dataTime) < payload_size:
    #   dataTime += c.s.recv(4096)

    # packed_msg_size2 = dataTime[:payload_size]
    # dataTime = dataTime[payload_size:]
    # msg_size2 = struct.unpack("Q", packed_msg_size)[0]

    # while len(dataTime) < msg_size:
    #   dataTime += c.s.recv(4096)
    
    # time_data = dataTime[:msg_size]
    # data = dataTime[msg_size2:]

    frame_data, data = retrieveData(data)
    time_data, data = retrieveData(data)

    print(int.from_bytes(time_data, 'big'))
